...
// MongoDB Connection
mongoose.connect(process.env.MONGODB_URI || 'mongodb://192.168.2.96:27017/notification-system', {
...
// Handle connections on root namespace
io.on('connection', (socket) => {
  console.log('=== New Socket Connection (Root) ===');
...
// Handle connections on /chat namespace
chatIo.on('connection', (socket) => {
  console.log('=== New Socket Connection (/chat) ===');
  console.log('Socket ID:', socket.id);
  console.log('Auth Data:', socket.handshake.auth);
  console.log('Query Data:', socket.handshake.query);
  ...
  if (userId) {
    connectedUsers.set(userId, socket.id);
    console.log(`[User Connection] User ${userId} connected with socket ${socket.id} (chat namespace)`);
    console.log('Current connected users:', Array.from(connectedUsers.entries()));
  } else {
    console.log('[Warning] Connection attempt without userId (chat namespace)');
  }

  // Handle connection errors
  socket.on('connect_error', (error) => {
    console.error('=== Socket Connection Error (chat namespace) ===');
    console.error('Socket ID:', socket.id);
    console.error('Error details:', error);
    socket.emit('error', { message: 'Connection error', error: error.message });
  });
  ...
  // Handle user connection
  socket.on('userConnected', async (data) => {
    console.log('=== User Connected Event (chat namespace) ===');
    console.log('Socket ID:', socket.id);
    console.log('Connection data:', data);
    ...
    try {
      ...
      if (userId) {
        connectedUsers.set(userId, socket.id);
        console.log(`[User Connected] User ${userId} connected with socket ${socket.id} (chat namespace)`);
        console.log('Updated connected users:', Array.from(connectedUsers.entries()));
        ...
        console.log(`[Room Auto-Join] Found ${rooms.length} rooms for user ${userId} (chat namespace)`);
        rooms.forEach(room => {
          socket.join(room._id.toString());
          console.log(`[Room Auto-Join] User ${userId} joined room: ${room._id} (chat namespace)`);
        });
      }
    } catch (error) {
      console.error('=== User Connection Error (chat namespace) ===');
      console.error('Socket ID:', socket.id);
      console.error('Error details:', error);
      socket.emit('error', { message: 'Error handling connection', error: error.message });
    }
  ...
  socket.on('joinRoom', async (data) => {
    try {
      ...
      socket.join(roomId);
      console.log(`User ${userId} joined room: ${roomId} (chat namespace)`);
      ...
      // Notify others in room using chatIo instead of io
      socket.to(roomId).emit('userJoined', {
        roomId,
        userId,
        timestamp: new Date()
      ...
      console.error('Error joining room (chat namespace):', error);
      socket.emit('roomJoined', { 
        roomId: data.roomId, 
        success: false, 
        error: error.message 
  ...
  // Handle messages
  socket.on('sendMessage', async (data) => {
    console.log('=== New Message Event (chat namespace) ===');
    console.log('Socket ID:', socket.id);
    console.log('Message data:', JSON.stringify(data, null, 2));
    ...
    try {
      ...
      for (const roomId of roomIds) {
        ...
        // Broadcast to room using chatIo instead of io
        const messageToEmit = {
          _id: message._id,
          room: roomId,
          sender: {
        ...
        console.log('[Message Broadcast] Emitting to room (chat namespace):', JSON.stringify(messageToEmit, null, 2));
        chatIo.to(roomId).emit('newMessage', messageToEmit);
      }
      ...
      console.error('=== Message Error (chat namespace) ===');
      console.error('Socket ID:', socket.id);
      console.error('Error details:', error);
      console.error('Failed message data:', data);
      socket.emit('messageSent', { 
  ...
  socket.on('markAsRead', async (data) => {
    try {
      ...
      // Notify others in room using chatIo
      socket.to(roomId).emit('messagesRead', {
        roomId,
        userId,
        timestamp: new Date()
      ...
      console.error('Error marking messages as read (chat namespace):', error);
      socket.emit('error', { 
        message: 'Failed to mark messages as read',
        error: error.message
      });
  ...
  socket.on('subscribeChatList', async (data) => {
    try {
      ...
      // Emit using chatIo for chat namespace
      socket.emit('chatListUpdate', {
        rooms: formattedRooms,
        timestamp: new Date()
      });
      ...
      console.error('Error subscribing to chat list (chat namespace):', error);
      socket.emit('error', { 
        message: 'Failed to subscribe to chat list',
        error: error.message
      });
  ...
  // Handle disconnection
  socket.on('disconnect', (reason) => {
    console.log('=== Socket Disconnection (chat namespace) ===');
    console.log('Socket ID:', socket.id);
    console.log('Disconnect reason:', reason);
    ...
    for (const [userId, socketId] of connectedUsers.entries()) {
      if (socketId === socket.id) {
        connectedUsers.delete(userId);
        console.log(`[User Disconnection] User ${userId} disconnected (chat namespace)`);
        console.log('Remaining connected users:', Array.from(connectedUsers.entries()));
        break;
      }
    }
  ...
  // Handle errors
  socket.on('error', (error) => {
    console.error('=== Socket Error (chat namespace) ===');
    console.error('Socket ID:', socket.id);
    console.error('Error details:', error);
    socket.emit('error', { message: 'An error occurred', error: error.message });
  });
